...
pub fn Play(#[prop(optional)] extend_tw_classes: &'static str) -> impl IntoView {
    ...
    let nav_buttons_style = "flex place-items-center justify-center hover:bg-green-300 my-1 h-6 rounded-md border-cyan-500 border-2 drop-shadow-lg";
    ...
    view! {
        <div class=move || {
            format!("max-h-[100dvh] min-h-[100dvh] pt-10 {} {extend_tw_classes}", parent_container_style())
        }>
            <Show
                when=is_tall
                fallback=move || {
